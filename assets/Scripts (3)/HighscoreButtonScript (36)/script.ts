...
class HighscoreButtonBehavior extends Sup.Behavior {
  ...
  mouse(action) {
    if(action == "click"){
      ...
      Sup.getActor("Highscores").textRenderer.setColor(1, 0, 0);
    }
    else if(action == "unhover"){
      Sup.getActor("Highscores").textRenderer.setColor(1, 1, 1);
    }
  }
...
Sup.registerBehavior(HighscoreButtonBehavior);